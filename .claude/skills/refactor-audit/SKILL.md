...
description: |
  ...
  crate単位でAgentを並列spawnし、10の検査次元×具体的チェックリストで検査。
...
5. crate単位でAgentを並列spawn（検査実行）
6. 各Agentの結果（JSON）を集約
7. Issue起票（選択された起票先へ）
...
スキャン開始前に `AskUserQuestion` で1回だけ聞く:
...
各Agentに渡す前に、関連する**仕様ファイル + 要件定義ファイル**の両方を読み、**そのcrateの実装に直接関係する部分だけを要約**（箇条書き、最大40行程度）して渡す。全文は渡さない。
...
## ステップ 5: Agent 並列 spawn

### spawn ルール

- **1 crate = 1 Agent**
- 全Agentを **同時に** spawn する（`Agent` ツールを複数同時に呼ぶ）
- 各Agentは **read-only**（Edit/Write/NotebookEdit は使わない）
- 各Agentには以下を渡す:
  - 対象crateの全ソースファイル一覧
...
1. 各Agentの出力からJSON配列をparse
2. 全結果を1つの配列にマージ
3. severity 順にソート（critical → major → minor → enhancement）
4. 連番を振る（001, 002, ...）

JSON parseに失敗した場合は、そのAgentの出力をそのままテキストとして扱い、手動で構造化するか、ユーザーに報告する。
...
- Agent が JSON parse 不能な出力を返した場合、ユーザーに報告して手動対応を促す
